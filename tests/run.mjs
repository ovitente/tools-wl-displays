...
// backend), and writes screenshots at each step.
//
...
function stubScript(fixture) {
  return `
    window.__calls = { apply: [], quit: false };
...
async function main() {
  ...
  // ---- 5. Apply → payload sent to backend ----
  ...
  check('Apply toast = "Configuration applied"', /Configuration applied/.test(await text(page, '#toast') || ''), await text(page, '#toast'));
  await page.screenshot({ path: join(SHOTS, '03-applied.png') });

  // ---- 5b. responsive: the card fills the window at any size ----
  ...
  // ---- 6. no console errors / no broken assets ----
